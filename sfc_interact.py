...
(function(){
  ...
  function initAll(){ try{initPan();}catch(e){} try{wireTrans();}catch(e){} }
...
def inject_interactions(sfc_html):
    """Insert the pan + transition-click script before </body> (or append)."""
    if not sfc_html:
        return sfc_html
    if '</body>' in sfc_html:
        return sfc_html.replace('</body>', INTERACT_JS + '</body>', 1)
    return sfc_html + INTERACT_JS
