...
def build_sfc_html(blocks, fname, opts=None):
    ...
    def _script_safe(s):
        return s.replace('</', '<\\/')   # prevent premature </script> termination
    js = _JS.replace('__DATA__', _script_safe(json.dumps(data_json))).replace('__EXTRA__', _script_safe(json.dumps(extra)))
